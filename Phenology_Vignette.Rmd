...
```{r echo = F}
save(mp, file = "Additional/Additional_Figure_05.RData")
```
...
```{r echo = F}
save(mp, file = "Additional/Additional_Figure_06.RData")
```
...
```{r echo = F}
save(mp, file = "Additional/Additional_Figure_09.RData")
```
...
[data/model_t+p.csv](https://github.com/derekmichaelwright/AGILE_LDP_Phenology/blob/master/data/model_t+p.csv)

[data/model_t+p_d.csv](https://github.com/derekmichaelwright/AGILE_LDP_Phenology/blob/master/data/model_t+p_d.csv)

[data/model_t+p_coefs.csv](https://github.com/derekmichaelwright/AGILE_LDP_Phenology/blob/master/model_t+p_coefs.csv)

[Additional/pdf_model.pdf](https://github.com/derekmichaelwright/AGILE_LDP_Phenology/blob/master/Additional/pdf_model.pdf)
...
[data/model_txp.csv](https://github.com/derekmichaelwright/AGILE_LDP_Phenology/blob/master/data/model_txp.csv)

[data/model_txp_d.csv](https://github.com/derekmichaelwright/AGILE_LDP_Phenology/blob/master/data/model_txp_d.csv)

[data/model_txp_coefs.csv](https://github.com/derekmichaelwright/AGILE_LDP_Phenology/blob/master/data/model_txp_coefs.csv)
...
[data/model_test.csv](https://github.com/derekmichaelwright/AGILE_LDP_Phenology/blob/master/data/data_pca_results.csv)

[data/model_test_d.csv](https://github.com/derekmichaelwright/AGILE_LDP_Phenology/blob/master/data/data_pca_results.csv)
...
xx <- read.csv("data/model_test_d.csv") %>% 
  mutate(Expt = factor(Expt, levels = names_Expt))
# Plot Observed vs Predicted
mp <- gg_model_2(xx, title = expression(paste("Overall: ", italic("R")^2, " = 0.804 | RMSE = 15.0")))
...
xx <- read.csv("data/data_tb_pc.csv") %>% 
  mutate(Expt = factor(Expt, levels = names_Expt))
# Plot (a)
mp1 <- gg_model_2(xx, "Tf", "predicted_Tf", "(a) Thermal sum required for flowering", 200, 200, 6600, 5500)
# Plot (b)
mp2 <- gg_model_2(xx, "DTF", "predicted_DTF_Tf", "(b) Days from sowing to flower", 30, 30, 145, 125)
# Append (a) and (b)
...
xx <- read.csv("data/data_tb_pc.csv") %>% 
  mutate(Expt = factor(Expt, levels = names_Expt))
# Plot (a)
mp1 <- gg_model_2(xx, "Pf", "predicted_Pf", "(a) Photoperiodic sum required for flowering", 190, 190, 1350, 1150)
# Plot (b)
mp2 <- gg_model_2(xx, "DTF", "predicted_DTF_Pf", "(b) Days from sowing to flower", 30, 30, 145, 125)
# Append (a) and (b)
mp <- ggarrange(mp1, mp2, nrow = 2, ncol = 1, common.legend = T, legend = "bottom")
...
[data/data_temp_phtoto_increase.csv](https://github.com/derekmichaelwright/AGILE_LDP_Phenology/blob/master/data/data_temp_phtoto_increase.csv)
