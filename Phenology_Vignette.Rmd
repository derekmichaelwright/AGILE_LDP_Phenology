...
```{r}
#install.packages(c("tidyverse","scales","rworldmap","ggrepel","magick","GGally",
#                   "ggbeeswarm","FactoMineR","plot3D","stringr"))
# Load libraries
...
```{r echo = F}
knitr::kable(s1[c(1:5,320:324),])
```

## Supplemental table 2

```{r}
s2 <- select(ff, Location, Year, `Short Name`=ExptShort, Latitude=Lat, Longitude=Lon,
...
x1.1 <- mypcaH[[1]] %>% rownames_to_column("Entry")
x1.2 <- mypca[[3]]$coord %>% as.data.frame() %>% rownames_to_column("Entry")
x1 <- left_join(x1.1, x1.2, by = "Entry") %>%
...
dev.off() #dev.set(dev.next())
# Create animation
xx <- dd %>% filter(ExptShort=="Su16") %>% arrange(DTF) %>% pull(Entry)
lf <- list.files("Additional/Entry_TP")[xx]
mp <- image_read(paste0("Additional/Entry_TP/", lf)) %>% image_scale("50%")
...
![](Additional/Entry_3D/3d_Entry_001.png)
...
mp <- gg_model_2(xx %>% filter(ExptShort %in% myexpts))
...
```{r echo = F}
image_read("Additional/Model/Model_2_3.png")
```
