...
xx <- xx %>% filter(Origin != "ICARDA") %>% 
  ...
  ungroup() %>% as.data.frame()
xx[is.na(xx)] <- 0 
invisible(png("Additional/Temp/Temp_F03_3.png", width = 2400, height = 1100, res = 150)) # 1200, 550
par(mai = c(0,0,0,0), xaxs = "i", yaxs = "i")
...
ggsave("Additional/Temp/Temp_SF04_1.png", mp, width = 6, height = 4)
# Append C)s
...
ggsave("Additional/Temp/Temp_SF04_3.png", mp3, width = 6, height = 3)
# Append A), B), C) and D)
im1 <- image_read("Additional/Temp/Temp_SF04_1.png") %>% 
  image_annotate("A)", size = 40, location = "+0+10") %>% 
  image_annotate("B)", size = 40, location = "+0+550")
...
```{r}
e1 <- ee %>% filter(ExptShort == "It17")
nrow(e1)
nrow(e1%>%filter(Temp_mean < 15))
```
...
# Plot A)
mp <- gg_model_2(xx) #title = expression(paste("Overall ",italic("R")^2, " = 0.804")
...
#
xx <- mt %>% 
  mutate(Model = paste(Temperate_Location, SouthAsian_Location, Mediterranean_Location, sep = "-")) %>%
  arrange(rev(RR)) %>%
  mutate(Model = factor(Model, levels = unique(Model)))
mp <- ggplot(xx, aes(x = Model, y = RR)) + 
  geom_bar(stat = "identity") + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
ggsave("Additional/Model_3_Locations.png", mp, width = 30, height = 5)
```
...
mp1 <- ggplot(xx, aes(x = Cluster, y = Value * 10000) ) + 
  ...
  theme(strip.text = element_text(face = "italic"),
        legend.margin     = unit(c(0,0,0,0), "cm"),
        plot.title = element_text(hjust = -0.085),
        legend.position = "none") +
  scale_fill_manual(name = NULL, values = colors) +
  guides(fill = F) +
  labs(title = "A)", y = "x 10000")
...
# Append ALL
mp <- ggarrange(mp1, mp2, nrow = 2, ncol = 1, align = "hv") #heights = c(1,1.75),
...
mp1 <- ggplot(x1, aes(x = ExptShort, y = Value)) +
  ...
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  guides(colour = guide_legend(nrow = 1, override.aes = list(size = 3))) +
  labs(title = "A)", y = expression(italic("T")[italic("f")]), x = NULL)
# Prep data for B) Pf
x1 <- xx %>% 
...
mp2 <- ggplot(x1, aes(x = ExptShort, y = Value)) +
  ...
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
  guides(colour = guide_legend(nrow = 1, override.aes = list(size = 3))) +
  labs(title = "B)", y = expression(italic("P")[italic("f")]), x = NULL)
...
#
#im1 <- image_read("Additional/Temp/Temp_SF09_1.png") %>% 
#  image_annotate("A)", size = 60, location = "+0+0")
#im2 <- image_read("Additional/Temp/Temp_SF09_2.png") %>% 
#  image_annotate("B)", size = 60, location = "+0+0")
#im3 <- image_read("Additional/Temp/Temp_SF09_3.png") %>%
#  image_annotate("C)", size = 60, location = "+0+0")
#im <- image_append(c(im1, im2), stack = T)
#image_write(im, "Supplemental_Figure_09_TfPcPTT.png")
