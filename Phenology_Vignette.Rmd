...
mp <- ggplot(xx, aes(x = ExptShort, y = Value)) + 
  ...
  theme_AGL + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  labs(x = NULL, y = NULL)
...
ggDistroDTF <- function(x) {
  mp <- ggplot(x, aes(x = Trait, y = Value) ) +
    ...
    theme_AGL + theme(axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
    labs(x = NULL, y = NULL)
  mp
}
...
gg_PTplane <- function(x, i, colored = T) {
  ...
  scatter3D(x, y, z, pch = 18, cex = 2, zlim = c(0.005,0.03), 
    ...
    xlab = "Temperature", ylab = "Photoperiod", zlab = "1 / DTF", main = unique(x1$Name),
    surf = list(x = x.pred, y = y.pred, z = z.pred, col = "black", 
                facets = NA, fit = fitpoints) )
}
# Plot each Entry
for (i in 1:324) {
  png(paste0("Additional/3D/3D_Entry_", stringr::str_pad(i, 3, pad = "0"), ".png"), 
...
# Create PDF
mp <- list()
pdf("Additional/Plots_3D.pdf") #, width = 600, height = 600
par(mar=c(1.5, 2.5, 1.5, 0.5))
for (i in 1:324) {
  gg_PTplane(xx, i)
}
...
for (i in c(76,94,128)) {
  png(paste0("Temp/3D_Entry_", stringr::str_pad(i, 3, pad = "0"), ".png"), 
      width = 600, height = 600, res = 150)
  gg_PTplane(xx, i)
  dev.off()
  #
  png(paste0("Temp/3D_Entry_", stringr::str_pad(i, 3, pad = "0"), "_BW.png"), 
      width = 600, height = 600, res = 150)
...
gg_Reg <- function(filename, colored = T) {
  ...
  mp1 <- ggplot(xx, aes(x = T_mean, y = RDTF)) +
    ...
    scale_y_continuous(trans = "reverse", breaks = c(0.01,0.02,0.03),
                      sec.axis = dup_axis(~ 1/., name = "DTF", 
                                          breaks = c(35,50,100,150))) +
    scale_x_continuous(breaks = c(11,13,15,17,19,21)) +
    ...
    facet_grid(.~Name) + guides(color = F) +
  ...
  # Append A) and B)
  mp <- ggarrange(mp1, mp2, ncol = 1, nrow = 2, label.y = "Test",
...
gg_model_1 <- function(x, title = NULL, xmin = min(x$DTF)-2, xmax = max(x$DTF)+2, 
                       ymin = min(x$Predicted_DTF)-2, ymax = max(x$Predicted_DTF)+2, type = 1) {
  # Prep data
  if(type == 1) {
    myx <- "DTF"
    myy <- "Predicted_DTF"
  ...
  mp <- ggplot(x) +
    ...
    theme_AGL + labs(y = "Predicted", x = "Observed")
...
gg_model_2 <- function(x, myX = "DTF", myY = "Predicted_DTF", title = NULL,
  ...
  mp <- ggplot(x, aes(x = get(myX), y = get(myY))) +
    ...
    geom_text(x = x1, y = y1, color = "black", hjust = 0, vjust = 0,
              aes(label = paste("RMSE = ", RMSE, sep = "")), size = 3, data = xf) +
    geom_text(x = x2, y = y2, color = "black", hjust = 0, vjust = 0,
              aes(label = paste("italic(R)^2 == ", r2)), size = 3, data = xf, parse = T) +
...
for(i in 1:324) {
  mp1 <- gg_model_1(mr %>% filter(Entry == i), paste("Entry", i, "| DTF"),
           xmin = min(x$DTF), xmax = max(x$DTF), 
           ymin = min(x$Predicted_DTF), ymax = max(x$Predicted_DTF))
  mp2 <- gg_model_1(mr %>% filter(Entry == i), paste("Entry", i, "| RDTF"), 
           xmin = min(x$RDTF), xmax = max(x$RDTF), 
           ymin = min(x$Predicted_RDTF), ymax = max(x$Predicted_RDTF), type = 2)
  mp <- ggarrange(mp1, mp2, ncol = 2, common.legend = T, legend = "right")
  fname <- paste0("Additional/Model/Model_Entry_", stringr::str_pad(i, 3, pad = "0"), ".png")
  ggsave(fname, mp, width = 12, height = 5) 
}
# Create pdf
pdf("Additional/Plots_Model.pdf",  width = 12, height = 5)
for(i in 1:324) { 
  mp1 <- gg_model_1(mr %>% filter(Entry == i), paste("Entry", i, "| DTF"),
           xmin = min(x$DTF)-2, xmax = max(x$DTF)+2, 
           ymin = min(x$Predicted_DTF)-2, ymax = max(x$Predicted_DTF)+2)
  mp2 <- gg_model_1(mr %>% filter(Entry == i), paste("Entry", i, "| RDTF"), 
           xmin = min(x$RDTF)-0.001, xmax = max(x$RDTF)+0.001, 
           ymin = min(x$Predicted_RDTF)-0.001, ymax = max(x$Predicted_RDTF)+0.001, type = 2)
  print(ggarrange(mp1, mp2, ncol = 2, common.legend = T, legend = "right"))
}
dev.off()
```
...
```{r}
# Prep data
xx <- read.csv("data/model_18_d.csv") %>% 
  mutate(Expt = factor(Expt, levels = names_Expt))
# Plot Observed vs Predicted
mp <- gg_model_1(xx)
...
```{r echo = F, out.width = "100%", height = 5}
mp
```
...
```{r echo = F, out.width = "100%", height = 6}
mp
```
...
```{r}
# Prep data
xx <- read.csv("data/model_16_d.csv") %>% 
  mutate(Expt = factor(Expt, levels = names_Expt))
# Plot Observed vs Predicted
...
```{r}
# Prep data 
xx <- read.csv("data/model_3_d.csv") %>% 
  mutate(Expt = factor(Expt, levels = names_Expt))
# Plot Observed vs Predicted
...
```{r}
# Append A) and B)
im1 <- image_read("Temp/Temp_F5_1.png") %>% 
  image_annotate("A)", size = 50)
im2 <- image_read("Temp/Temp_F5_2.png") %>% 
  image_annotate("B)", size = 50)
im <- image_append(c(im1, im2), stack = T)
...
xx <- bind_rows(x1, x2, x3, x4, x5) %>%
  ...
  mutate(Entry = factor(Entry),
         ...
         Expt = factor(Expt, levels = c("AGILE-18", "AGILE-3", "Roberts 1988",
                                        "Summerfield 1985 -V", "Summerfield 1985 +V")))
# Plot a, b and c
...
```{r echo = F, out.width = "100%", fig.height = 3}
mp
```
...
new.lab <- as_labeller(c(
  ...
  Tf="italic(T)[italic(b)]==-(italic(a)+italic(Tb))/italic(c)", Mediterranean="Mediterranean",
...
```{r echo = F, out.width = "100%", fig.height = 13.75}
mp
```
