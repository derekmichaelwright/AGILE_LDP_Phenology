...
This vignette contains the `R` code and analysis done for the paper: **Understanding photothermal interactions can help expand production range and increase genetic diversity of lentil (*Lens culinaris* Medik.)** 
...
#                   "GGally","ggbeeswarm","FactoMineR","plot3D","stringr",
#                   "htmlwidgets","plotly","leaflet","leaflet.minicharts"))
# Load libraries
...
# Prep data
x2 <- dd %>% left_join(select(pca, Entry, Cluster), by = "Entry") %>%
...
[Interactive Plot](https://derekmichaelwright.github.io/AGILE_LDP_Phenology/Additional/Additional_Figure_05.html)
...
mp <- plot_ly(xx, x = ~PC1, y = ~PC2, z = ~PC3, color = ~Cluster, colors = colors) %>%
  add_markers()
saveWidget(mp, file=normalizePath("Additional/Additional_Figure_05.html"))
```

## Additional Figure 6 PCA by Region

[Interactive Plot](https://derekmichaelwright.github.io/AGILE_LDP_Phenology/Additional/Additional_Figure_06.html)
...
xx <- read.csv("data/data_pca_results.csv") %>% 
  ...
  mutate(Cluster = factor(Cluster),
         Region = as.character(Region), Origin = as.character(Origin),
...
mp <- plot_ly(xx, x = ~PC1, y = ~PC2, z = ~PC3, color = ~Region, 
  ...
  add_markers() 
saveWidget(mp, file=normalizePath("Additional/Additional_Figure_06.html"))
```
...
```{r}
# Prep data
xx <- read.csv("data/data_pca_results.csv") %>% mutate(Cluster = factor(Cluster))
xx <- ldp %>% left_join(select(xx, Entry, Cluster), by = "Entry") %>%
  mutate(test1 = factor(paste(Origin, Cluster)))
...
[Interactive Plot](https://derekmichaelwright.github.io/AGILE_LDP_Phenology/Additional/Additional_Figure_09.html)
...
# Prep data
xx <- read.csv("data/data_pca_results.csv") %>% mutate(Cluster = factor(Cluster))
xx <- ldp %>% select(Entry, Name, Lat, Lon, Origin) %>% 
...
mp <- leaflet() %>% addProviderTiles(providers$CartoDB.Positron) %>%
      addMinicharts(lat = xx$Lat, lng = xx$Lon, width = 10,
                    fillColor = pal(xx$Cluster),
                    #colorPalette = xx$Cluster,
                    popup = popupArgs(supValues = xx[,1:6], showValues = F)) %>%
    addLegend("bottomleft", pal = pal, values = 1:8,
             title = "Cluster", opacity = 1 )
saveWidget(mp, file=normalizePath("Additional/Additional_Figure_09.html"))
```
...
# Plot Observed vs Predicted
mp1 <- gg_model_1(xx, 
  title = expression(paste("(a) ", italic(" 1 / f = a + bT + cP"))))
# Prep data
xx <- read.csv("data/model_txp_d.csv") %>% 
  mutate(Expt = factor(Expt, levels = names_Expt))
# Plot Observed vs Predicted
mp2 <- gg_model_1(xx, 
  title = expression(paste("(b) ", italic(" 1 / f = a + bT + cP + dTP"))))
# Append
...
mp <- ggarrange(mp1, mp2, ncol = 1, common.legend = T, legend = "bottom")
# Save
ggsave("Additional/Temp/Temp_SF09_1.png", mp1, 
       width = 10, height = 13 * 3 / 8.2, dpi = 600)
ggsave("Additional/Temp/Temp_SF09_2.png", mp2, 
       width = 10, height = 13 * 3 / 8.2, dpi = 600)
