...
```{r}
# Prep data
xx <- read.csv("data/data_tb_pc.csv") %>% #select(-MacroEnv) %>%
  mutate(Expt = factor(Expt, levels = names_Expt))
# Plot (a)
...
knitr::kable(x2 %>% group_by(MacroEnv) %>% 
  summarise(Min = round(min(Difference), 2), Max = round(max(Difference), 2)) )
xx <- bind_rows(x1, x2) %>%
  select(Expt, ExptShort, MacroEnv, Entry, Name, Cluster, DTF_0, DTF_1, Difference, Treatment)
write.csv(xx, "data/data_temp_phtoto_increase.csv", row.names = F)
new.lab <- as_labeller(c(`T + 1.5` = "italic(T)~+~1.5~degree*C", 
    `P + 0.1` = "italic(P)~+~0.1~italic(h)", Mediterranean = "Mediterranean", 
